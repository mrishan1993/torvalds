"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
exports.fill = void 0;
var tslib_1 = require("tslib");
var types_1 = require("@rarible/types");
var get_config_1 = require("../../common/collection/get-config");
var get_protocol_fee_1 = require("../get-protocol-fee");
var get_prepared_order_1 = require("../common/get-prepared-order");
var calculate_fees_1 = require("../../common/calculate-fees");
var transaction_1 = require("../../common/transaction");
var order_legacy_1 = require("../../tx-code-store/order/order-legacy");
var fill_bid_order_1 = require("./fill-bid-order");
function fill(fcl, auth, network, orderApi, itemApi, collection, currency, order, owner, originFee) {
    return (0, tslib_1.__awaiter)(this, void 0, void 0, function () {
        var from, _a, _b, _c, preparedOrder, _d, name_1, map, _e, txId, protocolFee, orderPayouts, payouts, filteredPayouts, royalties, _f, includedFees;
        return (0, tslib_1.__generator)(this, function (_g) {
            switch (_g.label) {
                case 0:
                    if (!fcl) return [3 /*break*/, 13];
                    if (!auth) return [3 /*break*/, 2];
                    _b = types_1.toFlowAddress;
                    return [4 /*yield*/, auth()];
                case 1:
                    _a = _b.apply(void 0, [(_g.sent()).addr]);
                    return [3 /*break*/, 4];
                case 2:
                    _c = types_1.toFlowAddress;
                    return [4 /*yield*/, fcl.currentUser().snapshot()];
                case 3:
                    _a = _c.apply(void 0, [(_g.sent()).addr]);
                    _g.label = 4;
                case 4:
                    from = _a;
                    if (!from) {
                        throw new Error("FLOW-SDK: Can't get current user address");
                    }
                    return [4 /*yield*/, (0, get_prepared_order_1.getPreparedOrder)(orderApi, order)];
                case 5:
                    preparedOrder = _g.sent();
                    _d = (0, get_config_1.getCollectionConfig)(network, collection), name_1 = _d.name, map = _d.map;
                    _e = preparedOrder.type;
                    switch (_e) {
                        case "LIST": return [3 /*break*/, 6];
                        case "BID": return [3 /*break*/, 8];
                    }
                    return [3 /*break*/, 12];
                case 6: return [4 /*yield*/, (0, transaction_1.runTransaction)(fcl, map, (0, order_legacy_1.getOrderCodeLegacy)(name_1).buy(fcl, currency, preparedOrder.id, owner), auth)];
                case 7:
                    txId = _g.sent();
                    return [2 /*return*/, (0, transaction_1.waitForSeal)(fcl, txId)];
                case 8:
                    protocolFee = get_protocol_fee_1.getProtocolFee.percents(network);
                    orderPayouts = preparedOrder.data.payouts;
                    payouts = !!orderPayouts.length ?
                        orderPayouts :
                        [{ account: from, value: (0, types_1.toBigNumber)("1.0") }];
                    filteredPayouts = payouts.filter(function (p) { return p.account !== owner; });
                    if (!(network === "emulator")) return [3 /*break*/, 9];
                    _f = { royalties: [] };
                    return [3 /*break*/, 11];
                case 9: return [4 /*yield*/, itemApi.getNftItemById({ itemId: preparedOrder.itemId })
                    /**
                     * fees included in price, royalties, originFees, protocolFees
                     */
                ];
                case 10:
                    _f = _g.sent();
                    _g.label = 11;
                case 11:
                    royalties = (_f).royalties;
                    includedFees = (0, tslib_1.__spreadArray)((0, tslib_1.__spreadArray)((0, tslib_1.__spreadArray)((0, tslib_1.__spreadArray)([], filteredPayouts, true), originFee, true), royalties, true), [
                        protocolFee.sellerFee,
                    ], false);
                    return [2 /*return*/, (0, fill_bid_order_1.fillBidOrder)(fcl, auth, currency, name_1, map, preparedOrder.id, owner, (0, calculate_fees_1.calculateFees)(preparedOrder.make.value, includedFees))];
                case 12: throw new Error("Unsupported order type: ".concat(preparedOrder.type));
                case 13: throw new Error("Fcl is required for purchasing");
            }
        });
    });
}
exports.fill = fill;
