...
var get_base_order_fee_1 = require("./order/get-base-order-fee");
...
function createRaribleSdk(ethereum, env, sdkConfig) {
    ...
    var send = partialCall((0, send_transaction_1.getSendWithInjects)({
        logger: {
    ...
    var filler = new fill_order_1.OrderFiller(ethereum, send, config, apis);
    var approveFn = partialCall(approve_1.approve, ethereum, send, config.transferProxies);
    var upsertService = new upsert_order_1.UpsertOrder(filler, checkLazyOrder, partialCall(approve_1.approve, ethereum, send, config.transferProxies), partialCall(sign_order_1.signOrder, ethereum, config), apis.order, ethereum, checkWalletChainId);
    ...
    var startAuctionService = new start_1.StartAuction(ethereum, config, approveFn, apis);
    var putAuctionBidService = new put_bid_1.PutAuctionBid(ethereum, config, approveFn, apis.auction);
    var buyOutAuctionService = new buy_out_1.BuyoutAuction(ethereum, config, approveFn, apis.auction);
    return {
        apis: apis,
        order: {
            ...
            cancel: partialCall(cancel_1.cancel, checkLazyOrder, ethereum, config.exchange, checkWalletChainId),
            getBaseOrderFee: get_base_order_fee_1.getBaseOrderFee.bind(null, config),
            getBaseOrderFillFee: filler.getBaseOrderFillFee,
        },
        auction: {
            start: startAuctionService.start,
            cancel: cancel_2.cancelAuction.bind(null, ethereum, config),
            finish: finish_1.finishAuction.bind(null, ethereum, config),
        ...
        balances: {
            getBalance: new balances_1.Balances(ethereum, apis.erc20Balance).getBalance,
