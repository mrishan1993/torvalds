"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
exports.testRoyaltiesProviderBytecode = exports.deployTestRoyaltiesProvider = exports.createTestRoyaltiesProviderContract = void 0;
var tslib_1 = require("tslib");
function createTestRoyaltiesProviderContract(web3, address) {
    return new web3.eth.Contract(testRoyaltiesProviderAbi, address);
}
exports.createTestRoyaltiesProviderContract = createTestRoyaltiesProviderContract;
function deployTestRoyaltiesProvider(web3) {
    return (0, tslib_1.__awaiter)(this, void 0, void 0, function () {
        var empty, address;
        return (0, tslib_1.__generator)(this, function (_a) {
            switch (_a.label) {
                case 0:
                    empty = createTestRoyaltiesProviderContract(web3);
                    return [4 /*yield*/, web3.eth.getAccounts()];
                case 1:
                    address = (_a.sent())[0];
                    return [2 /*return*/, empty.deploy({ data: exports.testRoyaltiesProviderBytecode }).send({ from: address, gas: 4000000, gasPrice: "0" })];
            }
        });
    });
}
exports.deployTestRoyaltiesProvider = deployTestRoyaltiesProvider;
var testRoyaltiesProviderAbi = [
    {
        inputs: [
            {
                internalType: "address",
                name: "token",
                type: "address",
            },
            {
                internalType: "uint256",
                name: "tokenId",
                type: "uint256",
            },
        ],
        name: "getRoyalties",
        outputs: [
            {
                components: [
                    {
                        internalType: "address payable",
                        name: "account",
                        type: "address",
                    },
                    {
                        internalType: "uint96",
                        name: "value",
                        type: "uint96",
                    },
                ],
                internalType: "struct LibPart.Part[]",
                name: "",
                type: "tuple[]",
            },
        ],
        stateMutability: "nonpayable",
        type: "function",
    },
];
exports.testRoyaltiesProviderBytecode = "";
